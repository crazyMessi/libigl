  ...
  Eigen::PlainObjectBase<DerivedWN>& WN)
{
    ...
    const Eigen::Matrix<real_ec,1,3> & anorm)->real_wn
  {
    ...
    {
      const typename RowVec::Scalar safe_norm = std::max(loc_norm, epsilon);
      return (loc(0)*anorm(0)+loc(1)*anorm(1)+loc(2)*anorm(2))
                                  /(PI_4*(safe_norm*safe_norm*safe_norm));
    }
  };
  ...
  Eigen::PlainObjectBase<DerivedWN>& WN)
{
  ...
  fast_winding_number(P,N,A,point_indices,CH,CM,R,EC,Q,beta,epsilon,WN);
}
...
IGL_INLINE void igl::fast_winding_number(
  ...
  Eigen::PlainObjectBase<DerivedWN>& WN)
{
  fast_winding_number(P,N,A,Q,2,2.0,1e-8,WN);
}
